    ...
    return {
        ...
        _tickCooldowns() {
            ...
            for (const p of this.snapshot.providers) {
                if (p.cooldown_remaining_s > 0)
                    p.cooldown_remaining_s = Math.max(0, p.cooldown_remaining_s - 1);
            }
            for (const k of this.snapshot.rate_limited_keys) {
                if (k.cooldown_remaining_s > 0)
                    k.cooldown_remaining_s = Math.max(0, k.cooldown_remaining_s - 1);
            }
            for (const m of this.snapshot.rate_limited_models) {
                if (m.cooldown_remaining_s > 0)
                    m.cooldown_remaining_s = Math.max(0, m.cooldown_remaining_s - 1);
            }
        },
        statusLabel(p) {
            if (!p.healthy)
