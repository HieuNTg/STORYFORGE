...
- **Stage Summary**: branch EPUB exporter now respects the 200-line rule with path-enumeration + CSS isolated in an internal assets module. Remaining 200–250L backlog: services/infra/database.py (250). Commit `426a874`.

## Cycle #44 — Split services/infra/database.py into manager + setup helpers

- **Task ID**: 44-database-setup-split
- **Agent**: eng-loop (Claude)
- **Task**: Bring services/infra/database.py (250L) under the 200-line rule with zero behavior change. Impact scan via Serena `get_symbols_overview` + Grep: `services.infra.database` is also reachable as `services.database` (aliased in services/__init__.py via `_alias`, verified `alias.get_engine is database.get_engine`). External callers use the stateful public API (`get_engine`, `get_session`, `transaction`, `init_db`, `close_db`); test patch targets are `services.database._session_factory` and `services.database.get_engine` (tests/test_db_integration.py) — a module global and a function that BOTH stay in database.py. The 4 stateless setup helpers (`_get_database_url`, `_setup_sqlite_pragmas`, `_setup_pool_metrics`, `_warn_no_db`) are never patched or imported externally and don't touch the `_engine`/`_session_factory` singletons. Split-safe.
- **Work Log**:
  - New `services/infra/_database_setup.py` (82L): the 4 stateless helpers moved verbatim (URL resolution, SQLite pragma + pool-metrics event listeners, no-DB warning); `sqlalchemy.event` stays a function-local import, `AsyncEngine` annotation moved under `TYPE_CHECKING`.
  - `database.py` (250→196L): keeps the singleton state (`_lock`, `_engine`, `_session_factory`) and the stateful public API; re-imports the 4 helpers from `_database_setup` (`# noqa: E402`, placed after the globals they reference) so `services.infra.database.<helper>` references/patch targets keep resolving. Dropped now-unused `import warnings`.
  - Targeted: tests/test_db_integration.py 12 passed; ruff check + format clean; smoke OK (all 9 public names present on the module; `services.database` alias identity holds).
  - Gate: EXIT 0/0/0/5(expected)/0, 4658 passed (= baseline+5 from intervening work; pure refactor adds 0), coverage 72.87% (= baseline 72.87%; floor 70.61%).
- **Loop retrospective**: Smooth (~30 min). Friction: stateful module — kept the globals + their mutators together and extracted only the stateless helpers. First pass landed at 204L (over) because of an unnecessary `__all__` block; removed it (original had none, the alias works by module identity) → 196L. Lesson for future stateful-module splits: budget for the re-import lines, skip a ceremonial `__all__` when the original lacked one.
- **Stage Summary**: database manager now respects the 200-line rule with stateless engine-setup helpers isolated in an internal module. The original 200–250L file-size backlog band is now CLEAR. Next band (250–270L): services/pipeline/smart_revision.py (262), pipeline/layer1_story/chapter_contract_builder.py (265), pipeline/layer1_story/consistency_validators.py (268). Commit `4a77bf2`.

## Cycle #45 — Split services/pipeline/smart_revision.py review helpers into internal module

- **Task ID**: 45-smart-revision-review-split
- **Agent**: eng-loop (Claude)
- **Task**: Bring services/pipeline/smart_revision.py (262L) under the 200-line rule with zero behavior change. `SmartRevisionService` is a single class (re-exported via services/pipeline/__init__.py + aliased as `services.smart_revision`). Its two tail helpers — `_aggregate_review_guidance` (36L) and `_find_chapters_with_agent_issues` (29L) — are pure functions (use only `re` + args, never `self`/`self.llm`/`self.scorer`). Extracted both to an internal module as module functions.
- **Work Log**:
  - New `services/pipeline/_smart_revision_review.py` (82L): `aggregate_review_guidance(chapter_number, reviews)` and `find_chapters_with_agent_issues(reviews, min_issues=3)`, bodies moved verbatim; owns the `re`/`AgentReview` imports.
  - `smart_revision.py` (262→198L): keeps `SmartRevisionService` + `revise_weak_chapters`, which now call the module functions directly; dropped now-unused `import re`.
  - **Mid-cycle fix (gate caught it)**: first pass removed both private methods outright → chunk 3 failed `tests/test_smart_revision.py::test_aggregate_review_guidance`, which calls `svc._aggregate_review_guidance(1, reviews)` on the *instance*. My class-level `find_referencing_symbols` scan missed it because the test references the method via an instance, not the class symbol. Restored a thin instance shim `_aggregate_review_guidance` (delegates to the module function) to preserve the test's contract; `_find_chapters_with_agent_issues` has no caller so no shim. Re-ran: green.
  - Targeted: tests/test_smart_revision.py 8 passed; tests/test_high_impact_coverage.py -k SmartRevision 2 passed; ruff check + format clean; smoke OK (module funcs return expected issues/sets; shim present, second helper gone).
  - Gate: EXIT 0/0/0/5(expected)/0, 4658 passed (= baseline), combined coverage 73% (≈ baseline 72.87%; floor 70.61%, hard gate cov-fail-under=60 — no chunk failed on coverage).
- **Loop retrospective**: Friction (recurring, now P2-worthy): a class-level `find_referencing_symbols` does NOT surface tests that call a method via an *instance variable* (`svc._method(...)`). Before removing/renaming ANY method — even a `_private` one — grep the tests dir for the bare method name too. Two cycles of split work relied on symbol-reference scans alone; this is the first method a test poked directly. **Action**: added to profile rule — "grep tests/ for the literal method/attr name before deleting it, in addition to Serena reference scan."
- **Stage Summary**: smart revision service now respects the 200-line rule with stateless agent-review aggregation isolated in an internal module. 250–270L band remaining: pipeline/layer1_story/chapter_contract_builder.py (265), pipeline/layer1_story/consistency_validators.py (268). Commit `06ce3ed`.

## Cycle #46 — Split pipeline/layer1_story/chapter_contract_builder.py into builder + assembly

- **Task ID**: 46-chapter-contract-assembly-split
- **Agent**: eng-loop (Claude)
- **Task**: Bring pipeline/layer1_story/chapter_contract_builder.py (265L) under the 200-line rule with zero behavior change. Free-function module. Impact scan via Serena overview + **tests/ grep for bare names** (new rule from #45): `build_contract`, `format_contract_for_prompt`, `validate_contract_compliance` are `patch()`-ed via `pipeline.layer1_story.chapter_contract_builder.<name>` (test_batch_generator, test_chapter_contract_setup, test_contract_batch_retry); `extract_chapter_num`/`events_for_chapter` are imported directly (test_handoff_builder). So ALL five names must stay attribute-accessible on the module → re-export required.
- **Work Log**:
  - New `pipeline/layer1_story/_chapter_contract_assembly.py` (157L): `extract_chapter_num`, `events_for_chapter`, `build_contract` + `_CHAPTER_TAG_RE` moved verbatim (with the `__future__`/`TYPE_CHECKING` schema-annotation block they need).
  - `chapter_contract_builder.py` (265→139L): keeps `format_contract_for_prompt` + `validate_contract_compliance` + `_VALIDATE_PROMPT`, re-imports the 3 moved names and lists all 5 in `__all__`. Dropped now-unused `re`/`Any`/`TYPE_CHECKING`/`__future__` imports.
  - **Split-design note**: deliberately kept the `format_contract_for_prompt` ↔ `validate_contract_compliance` pair together in the main module. `validate` calls `format` by bare name, and a test patches `chapter_contract_builder.format_contract_for_prompt`; co-locating them preserves the "patching the module attr also affects validate's internal call" semantics. The moved functions (`build_contract`, the event matchers) have NO internal callers in this module, so re-export is transparent for their patch targets.
  - Targeted: test_chapter_contracts + test_handoff_builder + test_chapter_contract_setup + test_contract_batch_retry + test_batch_generator = 77 passed (incl. all patch-target tests); ruff check + format clean; smoke OK (all 5 names present, extract_chapter_num('ch_10')→10).
...
## Cycle #48 - "Viết tiếp truyện" rewired from the cheap forge onto the real L1/L2 continuation pipeline

- **Task ID**: 48-continue-story-production
- **Agent**: Claude (CTO), CEO-directed ("làm cho nó chuẩn production")
- **Task**: The Continue screen was calling `/api/forge/sentence/stream` - the one-round-trip cheap-model forge. Audit confirmed it ran **zero** of the ~35 from-scratch pipeline stages. Move it onto the real continuation stack without abandoning the localStorage-only Library.
- **Work Log**:
  - **Symptom that started it**: HTTP 422 on every continue. `ContinueStoryScreen.buildIdea` flattened the story into a prompt and clamped it at 700 chars while `ForgeRequest.sentenceIdea` capped at 500. Raised the cap to 2000 as a stopgap (`models/schemas.py`, `frontend/types/story.ts`) - this unblocked the feature but did not make it correct.
  - **Real defects found**: (a) each chapter was a fresh forge call that invented 2 new characters and a new title, of which the client kept only `firstChapter` - so prose was anchored to characters that never entered the library; (b) context was 8 bare character *names* plus the last chapter truncated to 400 chars; (c) chapters written in the same batch could not see each other (`story` was a snapshot captured before the loop).
  - **Dead code path discovered**: `POST /api/pipeline/continue` had been broken since introduction - `continuation_routes` passed `arc_directives` through `PipelineOrchestrator.continue_story` -> `StoryContinuation.continue_story` -> `StoryGenerator.continue_story`, and neither of the outer two accepted the kwarg. Unconditional `TypeError` on every call; no frontend caller, so nobody noticed. Fixed both signatures.
  - **New bridge** `services/library_continuation.py`: hydrates a `PipelineOutput` from the client's story. Prefers the story's on-disk checkpoint (resolved by reproducing `CheckpointManager.save`'s title-hash filename, layer 2 then 1) so pipeline-born stories continue with their full L1 signals - voice profiles, macro arcs, conflict web, foreshadowing, story bible - and grafts the client's prose on top (client is authoritative for text; a longer checkpoint is never truncated by a stale client). Falls back to payload-only hydration for forge-born stories, synthesising chapter summaries so `rebuild_context` isn't fed blanks.
  - **New endpoint** `POST /api/pipeline/continue/library` (SSE, job-registry backed like `/continue`): hydrate -> clamp to `target_total_chapters` -> `orch.continue_story(direction=...)` -> optional L2 -> `done` carrying only the new chapters. A checkpoint is written as a side effect, so a story continued once becomes checkpoint-backed for every later continuation.
  - **L2 scoped to the tail** (`enhance_tail`): `StoryContinuation.enhance_chapters` enhances the whole story and checkpoints whatever draft it holds - wrong here on cost and on semantics (it would rewrite chapters the reader already has). L2 now runs against a copy holding only the new chapters, full cast/world included, and merges the enhanced prose back.
  - **User direction is now a real signal**: threaded `direction` through `generate_continuation_outlines` -> `continue_story` -> orchestrator, appended to the outline prompt like arc directives. Previously it was just text glued into a forge prompt.
  - **Two production bugs fixed while smoke-testing**: (1) `simulator.run_simulation_async` crashed with `AttributeError: 'str' object has no attribute 'get'` when the drama evaluator returned a list of strings - the handler caught only KeyError/ValueError/TypeError, so the whole L2 pass died. Added an isinstance guard + widened the except. (2) `enhance_tail` now swallows L2 failures and returns 0, because L1 chapters are already written and checkpointed - losing them to a simulator crash is worse than shipping un-enhanced prose.
  - **Environment fix**: `dev.ps1` sets `PYTHONNOUSERSITE=1`, which hid user-site numpy 1.26.4 and exposed system numpy 2.4.3 - incompatible with the locally installed torch 2.3.1, so `EmbeddingService` raised "Numpy is not available" and the semantic foreshadowing verifier + beat-coverage metrics silently degraded on *every* pipeline run. Aligned system numpy to <2.
- **Verification**: 3 live end-to-end runs against the real LLM. Run 1 (payload hydration): real outlines -> chapter -> context extraction -> consistency check, 726 words, direction honoured. Run 2: `hydration_source=checkpoint`, proving the checkpoint written by run 1 is reused. Run 3 (post-fix, L2 on): 47 L2 log lines through simulation -> enhancement -> consistency -> causal -> contract gate -> thread watchdog, `enhanced_chapters=1`. New tests: `tests/test_library_continuation.py` (20), `ContinueStoryScreen.test.tsx` (7), 3 simulator regression tests (verified failing without the guard). Frontend suite 112 passed; `tsc --noEmit` clean.
- **Backlog (next cycles)**:
  - **P1**: `pipelineSummaryToStory` drops per-chapter summaries and the whole character roster when saving a pipeline run to the Library - continuation has to re-derive both. Carry them through.
  - **P2**: `/api/pipeline/continue` (checkpoint-addressed) still has no frontend caller; decide whether to expose outline-preview/multi-path editing in the UI or retire the endpoints.
  - **P2**: `word_count` for continuation is hardcoded to 2000 client-side; expose it or derive from existing chapter lengths.

## Cycle #49 - Close the cycle #48 backlog: roster/summary passthrough, outline preview UI, real word-count control

- **Task ID**: 49-continue-backlog-closeout
- **Agent**: Claude (CTO), CEO-directed ("fix luôn em")
- **Task**: Clear all three backlog items #48 left open.
- **Work Log**:
  - **P1 - auto-save was lossy.** `build_output_summary` emitted characters as `{name, personality}` and no chapter summaries at all, so `pipelineSummaryToStory` persisted an empty roster and blank summaries. Every continuation of a pipeline-born story then had to re-derive both. Backend now emits the full sheet (`role`, `background`, `secret`, `motivation`, `internal_conflict`) plus `summary` on draft *and* enhanced chapters; the mapper carries them across, matching enhanced chapters back to draft summaries **by chapter number** (L2 rewrites prose without re-summarising, and the enhanced list is not guaranteed dense).
  - **Traits honesty.** The old mapper dropped the roster partly to avoid inventing `role`/traits. Role is now normalised (`chính`→protagonist, `phản diện`→antagonist, …, unknown→supporting: a wrong badge is recoverable, a dropped character is not). Traits have no L1 source, so `forgeCharacterSchema.traits` became `nullable` and `TraitRadar` renders an empty state instead of a radar of fabricated 50s. Fabricating four numbers and drawing them as a chart would have been a lie the UI tells confidently.
  - **Outline preview + edit (closes "endpoints with no UI").** New `POST /api/pipeline/continue/library/outlines` plans the next chapters without writing prose — measured live at **14s for 2 outlines vs ~85s per written chapter**, so the reader can steer before paying for prose. `/continue/library` now accepts `outlines`; when present it calls `write_from_outlines` instead of `continue_story`, so an edited plan is written verbatim rather than silently re-planned. Threaded `direction` through `generate_continuation_outlines` on both orchestrator layers to match. Target clamping moved into a shared `_clamp_to_target` used by both endpoints.
  - **word_count (was hardcoded 2000).** New `lib/library/chapter-length.ts` derives the default from the **median** of the last 5 chapters, rounded to 100 (median so one runaway chapter can't drag the target), clamped to the backend's 100..20000. Exposed as an editable field; the hint says whether the value is derived or manually set.
  - **Lint discipline**: the first cut of the outline panel reset state from an effect (`react-hooks/set-state-in-effect`). Replaced with a plan tagged by `storyId:chapterCount` — a stale plan simply stops applying, no cascading render, and outlines can never be written against a different batch than they were planned for.
- **Verification**: live runs against the real LLM — outline preview honoured the direction ("phiên toà giang hồ, bị vu oan") in 14s; a hand-edited outline ("Quán Trọ Đèn Vàng", new character Hàn Nguyệt, explicitly *no* trial) was written verbatim, confirming no re-plan. Tests: `test_library_continuation.py` 20→29, `test_output_summary.py` +3, `ContinueStoryScreen.test.tsx` 9→13, new `chapter-length.test.ts` (7), `story-mappers.test.ts` rewritten (the old test asserted the roster was empty — it now asserts the opposite). Frontend 130 passed; `tsc` clean; `ruff check` clean across api/pipeline/services/tests/models; no new eslint errors (3 pre-existing remain in ContinueStoryScreen, all in untouched code).
- **Backlog (next cycles)**:
  - **P2**: multi-path continuation (`/continue/paths`, `/continue/select-path`) still has no UI and no library-payload variant. Deliberate: outline preview covers the "see before you buy" need at 1x the cost; multi-path is 3-5x for a feature nobody has asked for yet. Decide to build or retire once the outline panel has usage.
  - **P2**: `scripts/run_gate_chunks.ps1` chunk 4 exits 5 ("no tests collected") and the script swallows it — the gate reports success on a chunk that ran nothing.
  - **P2**: combined coverage reads 69% under `STORYFORGE_DISABLE_REAL_EMBEDDINGS=1` vs the 72% recorded in #47; no baseline was captured under identical conditions, so the gap is unexplained rather than known-bad.
