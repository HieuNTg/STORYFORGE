...
class BaseAgent(ABC):
    ...
    def _format_round2_rebuttal_context(round2_entries, target_agent_name: str) -> str:
        ...
        for e in round2_entries:
            try:
                if getattr(e, "target_agent", "") == target_agent_name and str(
                    getattr(e, "stance", "")
        ...
        return "## Round 2 challenges against you (rebut directly):\n" + "\n".join(
            challenges_against_me[:6]
        )

    def _parse_debate_llm_response(
        self, result: dict, all_reviews: list[AgentReview]
    ) -> list[DebateEntry]:
        ...
        for raw in entries_raw:
            ...
            if len(target_issue) > 100:
                logger.debug(
                    "target_issue truncated from %d to 100 chars", len(target_issue)
                )
    ...
    def _parse_review_json(
        self, result: dict, layer: int, iteration: int
    ) -> AgentReview:
