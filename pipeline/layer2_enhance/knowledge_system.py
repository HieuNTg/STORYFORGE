...
class KnowledgeRegistry:
    ...
    def reveal_to(self, fact_id: str, char_name: str, round_num: int) -> None:
        """Tiết lộ một sự thật cho nhân vật. Cập nhật known_by."""
        item = self.items.get(fact_id)
        if item is None:
            return
    ...
    def check_revelation_triggers(self, posts: list, round_num: int) -> list[dict]:
        ...
        for post in posts:
            ...
            for item in secret_items:
                ...
                if match_count < 2:
                    continue
                # Post này tiết lộ bí mật — cập nhật tất cả nhân vật trong cuộc trò chuyện
                if post.target and post.target not in item.known_by:
                    self.reveal_to(item.fact_id, post.target, round_num)
                    revelations.append({
