"""Plugin loader for StoryForge.

Scans the plugins/ directory for Python files, imports each one, and calls
register() on any StoryForgePlugin subclass found inside. Plugins are indexed
by name for hook dispatch.

Usage:
    from plugins import plugin_manager

    # Apply on_genre_rules hooks
    rules = plugin_manager.apply_genre_rules(genre, rules)

    # Apply on_score hooks
    scores = plugin_manager.apply_score(scores)

    # Apply on_export hooks
...
class PluginManager:
    ...
    def load_all(self, directory: Path = _PLUGINS_DIR) -> None:
        """Scan directory for plugin modules and register all StoryForgePlugin subclasses."""
