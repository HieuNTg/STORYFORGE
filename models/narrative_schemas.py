...
class ArcWaypoint(BaseModel):
    ...
    stage_name: str = Field(
        description="Tên giai đoạn: 'phủ nhận', 'thử thách', 'khủng hoảng', 'chuyển hóa'..."
    )
    ...
    progress_pct: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Tiến trình arc 0.0-1.0"
    )
...
class ChapterContract(BaseModel):
    ...
    chapter_number: int
    must_advance_threads: list[str] = Field(
        default_factory=list, description="Thread IDs that MUST progress"
    )
    must_plant_seeds: list[str] = Field(
        default_factory=list, description="Foreshadowing hints to plant"
    )
    must_payoff: list[str] = Field(
        default_factory=list, description="Foreshadowing payoffs due"
    )
    character_arc_targets: dict[str, str] = Field(
        default_factory=dict, description="{name: 'stage (pct%)'}"
    )
    pacing_type: str = Field(default="rising", description="Expected pacing type")
    emotional_endpoint: str = Field(
        default="", description="Target emotional state at chapter end"
    )
    must_mention_characters: list[str] = Field(
        default_factory=list, description="Characters that MUST appear"
    )
    previous_contract_failures: list[str] = Field(
        default_factory=list, description="Missed items from previous chapter"
    )
    forbidden_actions: list[str] = Field(
        default_factory=list, description="Actions that MUST NOT happen"
    )
    must_maintain: list[str] = Field(
        default_factory=list, description="Facts/states that must remain true"
    )
    world_rules: list[str] = Field(
        default_factory=list, description="World rules to enforce"
    )
    secret_protection: dict[str, str] = Field(
        default_factory=dict, description="{char: secret} - secrets to guard"
    )
    causal_dependencies: list[str] = Field(
        default_factory=list, description="Prior events that MUST be acknowledged"
    )
