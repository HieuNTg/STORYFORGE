...
class EPUBExporter:
    ...
    def export(
        ...
        title_page = epub.EpubHtml(
            title="Trang bìa", file_name="title.xhtml", lang=language
        )
...
{f'<p class="synopsis">{story.synopsis[:500]}</p>' if hasattr(story, "synopsis") and story.synopsis else ""}
        ...
        if characters:
            char_page = epub.EpubHtml(
                title="Nhân vật", file_name="characters.xhtml", lang=language
            )
            char_lines = []
            for c in characters:
                char_lines.append(
            ...
            char_page.content = f"<html><body><h2>Nhân vật</h2>{''.join(char_lines)}</body></html>".encode(
                "utf-8"
            )
        ...
        for ch in story.chapters:
            ...
            paragraphs = [
                f"<p>{p.strip()}</p>" for p in ch.content.split("\n") if p.strip()
            ]
