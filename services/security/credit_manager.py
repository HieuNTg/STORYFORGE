...
class CreditManager:
    ...
    def deduct_credits(self, profile: UserProfile, action: str) -> tuple[bool, str]:
        ...
        if cost > 0:
            ...
            logger.info(
                f"Deducted {cost} credits from {profile.username}. Remaining: {profile.credits}"
            )
    ...
    def check_chapter_limit(
        self, profile: UserProfile, requested_chapters: int
    ) -> tuple[bool, str]:
        ...
        if requested_chapters > max_ch:
            return (
                False,
                f"Tier {profile.tier}: tối đa {max_ch} chương. Nâng cấp để mở khóa.",
            )
        return True, ""
