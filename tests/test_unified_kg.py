...
def _make_unified_draft():
    ...
    thread_open = types.SimpleNamespace(
        thread_id="t1",
        description="Lost heirloom",
        status="open",
        planted_chapter=1,
        involved_characters=["Linh"],
...
class TestEdgeType(unittest.TestCase):
    ...
    def test_normalize_unknown_falls_back(self):
        self.assertEqual(
            _normalize_edge_type("totally-bogus"), EdgeType.RELATED_TO.value
        )
...
class TestBuildUnified(unittest.TestCase):
    ...
    def test_resolved_thread_uses_resolves_edge(self):
        ...
        self.assertTrue(
            any(e["type"] == EdgeType.RESOLVES_THREAD.value for e in edges_to_t2)
        )
    ...
    def test_open_thread_uses_advances_edge(self):
        ...
        self.assertTrue(
            any(e["type"] == EdgeType.ADVANCES_THREAD.value for e in edges_to_t1)
        )
    ...
    def test_concurrent_mutation_no_crash(self):
        ...
        def _worker(i: int) -> None:
            try:
                for j in range(50):
                    kg.add_relationship(
                        "char:A", "char:B", EdgeType.ALLY.value, chapter=i * 100 + j
                    )
