...
class TestFillL2Portion:
    def test_filling_l2_preserves_l1(self):
        c = NegotiatedChapterContract(
            chapter_num=5,
            pacing_type="climax",
            threads_advance=["thread_revenge"],
            payoffs_required=["bí mật"],
    ...
    def test_snapshot_three_chapter_story(self):
        contracts = [
            NegotiatedChapterContract(
                chapter_num=i,
                pacing_type=p,
                threads_advance=[f"t{i}"],
                must_mention_characters=["Hoa", "Minh"],
        ...
        for c in contracts:
            l2 = {
                "drama_target": 0.4
                if c.pacing_type == "setup"
                else 0.6
                if c.pacing_type == "rising"
                else 0.85,
        ...
        assert not any(
            "payoffs_required" in w for w in filled[2].reconciliation_warnings
        )
