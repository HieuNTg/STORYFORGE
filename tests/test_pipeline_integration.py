...
def test_layer2_receives_draft_from_layer1(
    minimal_draft, minimal_enhanced, minimal_sim
):
    ...
    sim = mock_simulator.run_simulation(
        characters=minimal_draft.characters,
        relationships=analysis["relationships"],
        genre=minimal_draft.genre,
        num_rounds=3,
    )
    ...
    assert (
        mock_enhancer.enhance_with_feedback.call_args.kwargs["draft"] is minimal_draft
    )


def test_full_pipeline_output_has_all_layers(
    minimal_draft, minimal_enhanced, minimal_sim
):
    output = PipelineOutput(
        story_draft=minimal_draft,
        enhanced_story=minimal_enhanced,
...
def test_resume_from_layer1_runs_layer2(
    tmp_path, minimal_draft, minimal_enhanced, minimal_sim
):
    from pipeline.orchestrator_checkpoint import CheckpointManager
    import pipeline.orchestrator_checkpoint as ckpt_mod

    output = PipelineOutput(
        story_draft=minimal_draft, current_layer=1, status="running"
    )
