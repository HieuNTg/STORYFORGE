...
def _make_draft(n_chapters=2) -> StoryDraft:
    ...
    return StoryDraft(
        title="Test Story", genre="fantasy", chapters=chapters, characters=characters
    )
...
class TestLayerTransitions:
    def test_empty_draft_stops_pipeline(self):
        ...
        with (
            patch(
                "services.llm_client.LLMClient.check_connection",
                return_value=(True, "ok"),
            ),
            patch.object(orch.story_gen, "generate_full_story") as mock_gen,
        ):
            mock_gen.return_value = StoryDraft(title="Empty", genre="test", chapters=[])
            result = asyncio.run(
    ...
    def test_layer2_fallback_preserves_chapters(self):
        ...
        with (
            ...
            patch.object(
                orch.analyzer, "analyze", side_effect=Exception("Analysis failed")
            ),
        ):
            result = asyncio.run(
                orch.run_full_pipeline(
