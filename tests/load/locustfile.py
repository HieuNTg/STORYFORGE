...
class ConfigUser(HttpUser):
    ...
    def update_temperature(self):
        self.client.put(
            "/api/config", json={"temperature": round(random.uniform(0.6, 1.0), 1)}
        )
...
class ExportUser(HttpUser):
    ...
    def export_pdf_no_session(self):
        ...
        with self.client.post(
            f"/api/export/pdf/{fake_session}", catch_response=True
        ) as resp:
...
class MixedUser(HttpUser):
    ...
    def write_config(self):
        self.client.put(
            "/api/config", json={"temperature": round(random.uniform(0.5, 0.9), 1)}
        )
    ...
    def apply_preset(self):
        ...
        with self.client.post(
            f"/api/config/presets/{key}", catch_response=True
        ) as resp:
