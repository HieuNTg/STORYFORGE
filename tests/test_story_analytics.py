...
import os
import pytest
from models.schemas import Chapter, StoryDraft
...
class TestAnalyticsEdgeCases:
    ...
    def test_epub_export_vietnamese_diacritics_in_title(self, tmp_path):
        ...
        story = StoryDraft(
            ...
            chapters=[
                Chapter(
                    chapter_number=1, title="Khởi Đầu", content="Nội dung chương 1."
                )
            ],
        )
    ...
    def test_story_analytics_handles_vietnamese_diacritics_title(self):
        ...
        ch = make_chapter(
            1, "Nội dung chương với ký tự tiếng Việt đầy đủ.", title="Khởi Đầu Mới"
        )
        stats = StoryAnalytics.analyze_chapter(ch)
